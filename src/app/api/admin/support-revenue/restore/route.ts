...
import { restoreSupportRevenueDb, getSupportRevenueByIdDb } from '@/lib/support-revenue-db';
...
export const POST = withAdmin(
  async (req: NextRequest, admin) => {
    ...
    const prev = await getSupportRevenueByIdDb(body.id);
    ...
    const ok = await restoreSupportRevenueDb(body.id);
  ...
  { rate: { limit: 60, windowMs: 60_000 }, tag: 'support-revenue-restore' },
);

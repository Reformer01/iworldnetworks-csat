...
export default function AdminInstallation() {
  ...
  // Fetch real technicians and installation feedback
  const techQuery = useMemo(() => firestore ? query(collection(firestore, 'technicians'), orderBy('name')) : null, [firestore]);
  const installFeedbackQuery = useMemo(() => firestore ? query(collection(firestore, 'feedbacks'), orderBy('timestamp', 'desc'), limit(100)) : null, [firestore]);
  ...
  const techLeaderboard = useMemo(() => {
    ...
    return techs.map((t: any) => {
      ...
      return {
        ...t,
        completions,
        time: '42m' // Placeholder for now until we track completion times
      };
    ...
    <AdminLayout>
      <div className="grid grid-cols-12 gap-gutter mb-16">
        <div className="col-span-12 md:col-span-7">
          <h1 className="font-headline text-headline-lg mb-4 text-primary">Team Performance</h1>
          <p className="font-body-lg text-body-lg text-on-surface-variant max-w-xl">
            Detailed breakdown of setup speed, success rates, and regional coverage for our technical field teams.
          </p>
        </div>
        <div className="col-span-12 md:col-span-4 md:col-start-9 flex flex-col justify-end">
          <div className="bg-surface-container-lowest p-6 whisper-shadow rounded-xl border border-border">
            <span className="font-mono text-[12px] uppercase text-secondary">Success Rate</span>
            ...
            <div className="w-full bg-surface-container h-1 rounded-full mt-4">
              <div className="bg-secondary h-1 rounded-full" style={{ width: `${completionRate}%` }}></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-gutter">
        <section className="col-span-12 md:col-span-5 bg-surface-container-lowest whisper-shadow rounded-xl p-8 border border-border">
          <div className="flex justify-between items-center mb-10">
            <h3 className="font-headline text-[24px] font-bold">Technician Ranking</h3>
            <CircleCheck className="w-6 h-6 text-secondary" />
          </div>
          <div className="space-y-8">
            {techLeaderboard.map((tech: any) => (
              <div key={tech.id} className="flex items-center gap-6 group hover:scale-[1.01] transition-transform duration-200">
                ...
                <div className="flex-1">
                  ...
                  <p className="font-mono text-sm text-on-surface-variant">{tech.region} Hub</p>
                </div>
                <div className="text-right">
                  <p className="font-black text-secondary text-xl"><span className="font-mono">{tech.completions}</span></p>
                  <p className="font-mono text-[10px] uppercase opacity-60">Jobs</p>
                </div>
              </div>
            ))}
            {techLeaderboard.length === 0 && <p className="text-on-surface-variant text-sm font-mono">No data collected yet.</p>}
            <button className="flex items-center w-full gap-6 opacity-60 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-300">
              <div className="w-12 h-12 rounded-full flex items-center justify-center border-2 border-dashed border-border font-mono text-on-surface-variant">
                +{Math.max(0, (techs?.length || 0) - 5)}
              </div>
              <p className="font-mono text-sm">View Full Roster</p>
        ...
        <section className="col-span-12 md:col-span-7 bg-surface-container-lowest whisper-shadow rounded-xl p-8 border border-border flex flex-col min-h-[500px]">
          <div className="flex justify-between items-start mb-8">
            <div>
              <h3 className="font-headline text-[24px] font-bold">Regional Service Spots</h3>
              <p className="font-mono text-sm text-on-surface-variant mt-1">Live installation activity across our national hubs.</p>
            </div>
          ...
          <div className="flex-1 relative overflow-hidden rounded-lg bg-surface-container-low group">
            ...
            />
            {/* Real-time markers would go here based on data */}
          </div>
        </section>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter mt-16 mb-24">
        {[
          { label: 'Avg Setup Time', val: '45m', info: 'Daily Target', icon: Clock },
          { label: 'Quality Score', val: `${completionRate}%`, info: 'National Average', icon: CircleCheck },
          { label: 'Total Installs', val: (installFeedback?.filter((f: any) => f.category === 'Installation').length || 0).toString(), info: 'Past 30 Days', icon: List, primary: true },
        ].map((item, i) => (

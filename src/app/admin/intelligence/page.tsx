        ...
        <div className="max-w-screen-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 2xl:grid-cols-8 gap-3 mb-4">
            <AnimatedKpi
              ...
              color="bg-slate-900"
              sub="deleted=false"
              samples={[40, 65, 55, 80, 60, 90, 70, 85, 75, 95, 60, 80]}
            />
            <AnimatedKpi
              label="Active"
              value={data.totals.active}
              sub={`${data.lifecycle[0].percent}% · ${fmtNaira(data.totals.activeMrr)}`}
              icon={CheckCircle2}
            ...
            <AnimatedKpi
              ...
              sub={`${data.totals.total ? Math.round((data.totals.online / data.totals.total) * 10) / 10 : 0}% online`}
              icon={Zap}
              color="bg-sky-600"
            />
            <AnimatedKpi
            ...
            <DialogContent className="max-w-lg rounded-2xl">
              <DialogHeader>
                ...
                <DialogDescription className="font-mono text-[11px]">
                  Region {selectedBts?.region} · {selectedBts ? fmt(selectedBts.customers) : 0} customers · {selectedBts?.percent}% of
                  network
                </DialogDescription>
              </DialogHeader>
              {selectedBts && (
                <div className="space-y-3 font-mono text-xs">
                  ...
                  <p className="text-[11px] leading-relaxed opacity-70">
                    Every customer on this tower has an MRR from Splynx (servicePlan → monthly price). Inactive/blocked customers contribute
                    0 to active MRR but are still counted in total. Share = this tower’s customers ÷ {fmt(data.totals.total)} total — same

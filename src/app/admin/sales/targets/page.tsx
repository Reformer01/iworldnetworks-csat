...
export default function SalesTargets() {
  ...
  const [customTargets, setCustomTargets] = useState<
    { id?: string; month: string; region: string; targetRevenue: number; targetCustomers: number }[]
  >([]);
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewTarget({ ...newTarget, targetRevenue: e.target.value === '' ? 0 : Number(e.target.value) })}
                  />
                </div>
                <div>
                  ...
                  <Input
                    className="rounded-xl mt-1"
                    type="number"
                    value={newTarget.targetCustomers || ''}
                    onFocus={(e) => e.target.select()}
                    onChange={(e) => setNewTarget({ ...newTarget, targetCustomers: e.target.value === '' ? 0 : Number(e.target.value) })}
                  />
                </div>
                <Button
                  ...
                  return (
                    <div key={rt.region} className="border border-border rounded-2xl p-6">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="font-mono text-[10px] bg-secondary/10 text-secondary px-3 py-1 rounded-full font-bold">
                          {rt.targetPercentage}% of total
                        </span>
                      </div>
                      <div className="mb-4">
                        <div className="flex justify-between items-center mb-1.5">
                          ...
                          <span className="font-mono text-[9px] font-bold text-on-surface-variant">
                            {formatNaira(currentMrr)} / {formatNaira(rt.annualRevenueTarget)}
                          </span>
                        </div>
                      ...
                      <div className="mt-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {salesAgents
                            .filter((a) => a.region === rt.region)
                            .map((a) => (
                              <span key={a.name} className="font-mono text-[10px] bg-muted px-3 py-1 rounded-full font-bold">
                                {a.name} (₦{a.annualTarget.toLocaleString()})
                              </span>
                            ))}
                        </div>
                      </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-border/40 font-body text-sm">
                        {customTargets.map(
                          (t: { id?: string; month: string; region: string; targetRevenue: number; targetCustomers: number }) => (

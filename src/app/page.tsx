...
export default function LandingPage() {
  ...
  return (
    <div className="canvas-bg min-h-screen font-body overflow-x-hidden">
      ...
      <footer className="bg-surface-bright border-t border-border py-8 md:py-12">
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop flex flex-col md:flex-row justify-between items-start gap-8">
          <div className="space-y-4">
            <div className="font-mono text-label-mono font-bold text-primary uppercase text-xs md:text-sm">I-World Networks</div>
            <p className="text-on-surface-variant text-xs md:text-sm max-w-xs opacity-70">Reliable connections for your home and business in Nigeria.</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 md:gap-x-12 gap-y-4">
            {validRegions.map(link => (
              <a key={link} className="text-on-surface-variant hover:text-secondary transition-colors font-mono text-label-mono text-xs" href="#">{link}</a>
            ))}
          </div>
        </div>
      </footer>

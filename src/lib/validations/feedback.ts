...
export const feedbackSchema = z.object({
  customerName: z.string().min(1, 'Name is required').max(100, 'Name must be under 100 characters').trim().optional().default(''),
  customerEmail: z.string().email('Invalid email address').trim().toLowerCase().optional().default(''),
  ...
  ratings: z
    .object({
      ...
      responseTime: z.enum(['5min', '10min', '15min']).optional(),

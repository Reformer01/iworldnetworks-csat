...
const SENSITIVE_FILES = [/\.env$/, /\.env\./, /credentials/i, /secrets/i, /\.pem$/, /\.key$/, /id_rsa/, /id_ed25519/];
  ...
  return {
    'tool.execute.before': async (input, output) => {
      ...
      if (filePath && isSensitiveFile(filePath)) {
        throw new Error(`SECURITY BLOCK: Cannot edit sensitive file "${filePath}". Edit manually if needed.`);
      }
    },
  };
};
